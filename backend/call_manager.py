...
class CallManager:
    ...
    def _get_availability_context(self) -> str:
        """Generate a real-time availability summary for injection into LLM context."""
        today = date.today()
        lines = ["REAL-TIME APPOINTMENT AVAILABILITY:"]
        for i in range(5):
            day = today + timedelta(days=i)
            day_name = day.strftime("%A %B %d")
            hours = self.appointment_scheduler.get_business_hours(day)
            if not hours:
                lines.append(f"  {day_name}: CLOSED")
                continue
            slots = self.appointment_scheduler.get_available_slots(day)
            available = [s["start"] for s in slots if s["available"]]
            if available:
                lines.append(
                    f"  {day_name} ({hours['open']}-{hours['close']}): "
                    f"Available slots: {', '.join(available)}"
                )
            else:
                lines.append(f"  {day_name}: FULLY BOOKED")
        return "\n".join(lines)
    ...
    async def process_caller_text(self, call_id: str, caller_text: str) -> dict:
        ...
        # Generate AI response with real-time availability context
        availability_context = self._get_availability_context()
        response_text = await self.ai_engine.generate_response(
            active_call.conversation_history[:-1],  # History without current message
            caller_text,
            extra_context=availability_context,
        )
